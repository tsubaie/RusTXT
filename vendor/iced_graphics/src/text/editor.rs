...
impl Editor {
    ...
    /// Returns the viewport's logical line and pixel offsets.
    pub fn scroll_position(&self) -> (usize, f32, f32) {
        let scroll = self.buffer().scroll();
        (scroll.line, scroll.vertical, scroll.horizontal)
    }

    /// Restores a viewport after the widget has received its layout bounds.
    pub fn set_scroll_position(&mut self, line: usize, vertical: f32, horizontal: f32) {
        self.with_internal_mut(|internal| {
            // Shape the cursor first, then restore the independently scrolled viewport.
            let mut fonts = text::font_system().write().expect("Write font system");
            internal.editor.shape_as_needed(fonts.raw(), false);
            let buffer = buffer_mut_from_editor(&mut internal.editor);
            buffer.set_scroll(cosmic_text::Scroll { line: line.min(buffer.lines.len().saturating_sub(1)), vertical, horizontal });
            buffer.shape_until_scroll(fonts.raw(), false);
        });
    }
...
impl editor::Editor for Editor {
    ...
    fn with_text(text: &str) -> Self {
        ...
        // Bound initial layout before loading text; an unbounded height shapes the entire file.
        buffer.set_size(font_system.raw(), Some(1.0), Some(1.0));
        buffer.set_text(
            font_system.raw(),
            text,
    ...
    fn selection(&self) -> editor::Selection {
        ...
        let cursor = match internal.editor.selection_bounds() {
            Some((start, end)) => Selection::Range(buffer.layout_runs().filter_map(|run| {
                let (x, width) = run.highlight(start, end)?;
                (width > 0.0).then_some(Rectangle { x, y: run.line_top, width, height: run.line_height })
            }).collect()),
            None => {
                // COSMIC positions the caret using glyph coordinates and bidi levels.
                // Summing glyph widths places RTL and mixed-script carets incorrectly.
                let (x, y) = internal.editor.cursor_position().unwrap_or((-1, -1));
                Selection::Caret(Point::new(x as f32, y as f32))
            }
        };
