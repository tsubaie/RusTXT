...
impl Engine {
    ...
    pub fn draw_quad(
        ...
        let visible_bounds = if shadow.color.a > 0.0 {
            physical_bounds.union(&shadow_bounds)
        } else {
            physical_bounds
        };
        ...
        // Shadows extend beyond the quad and must always respect the current
        // layer/damage mask. Painting an unclipped shadow on a partial redraw
        // repeatedly darkens pixels that were not cleared for this frame.
        let shadow_clip_mask = Some(&*clip_mask);
        let clip_mask = (!physical_bounds.is_within(&clip_bounds)).then_some(&*clip_mask);
        ...
        if let Some(shadow_bounds) = (shadow.color.a > 0.0)
            .then(|| shadow_bounds.intersection(&clip_bounds))
            .flatten()
            .and_then(|bounds| {
        ...
        {
            // Rasterize only the damaged part, not an entire dialog-sized
            // shadow for each hovered control. Clamp before integer conversion
            // so shadows crossing a window edge retain the correct origin.
            let radii = fill_border_radius
                .into_iter()
                .map(|radius| radius * transformation.scale_factor())
                .collect::<Vec<_>>();
            let x = shadow_bounds.x.floor() as u32;
            let y = shadow_bounds.y.floor() as u32;
            let width = (shadow_bounds.x + shadow_bounds.width).ceil() as u32 - x;
            let height = (shadow_bounds.y + shadow_bounds.height).ceil() as u32 - y;
