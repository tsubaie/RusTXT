...
impl Content<crate::Renderer> {
    /// Returns the logical line and pixel offsets of the viewport.
    pub fn scroll_position(&self) -> (usize, f32, f32) {
        self.0.borrow().editor.scroll_position()
    }

    /// Restores the viewport after layout.
    pub fn set_scroll_position(&mut self, line: usize, vertical: f32, horizontal: f32) {
        self.0.get_mut().editor.set_scroll_position(line, vertical, horizontal);
    }
}
